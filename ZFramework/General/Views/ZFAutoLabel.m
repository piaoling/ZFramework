...
- (void) initAttributedString
{
    ...
    if(self.textAlignment == NSTextAlignmentCenter || self.textAlignment == NSTextAlignmentCenter)
    {
        alignment = kCTCenterTextAlignment;
    }
    if(self.textAlignment == NSTextAlignmentRight || self.textAlignment == NSTextAlignmentRight)
    {
...
-(void) drawTextInRect:(CGRect)requestedRect
{
    ...
    if (attributedString) {
        ...
        CGPathAddRect(leftColumnPath, NULL ,CGRectMake(0 , (self.bounds.size.height - requestedRect.size.height) / 2.0 ,self.bounds.size.width , self.bounds.size.height));
...
- (void)autoResetHeightByWidth:(CGFloat)width text:(NSString *)text lineSpace:(CGFloat)lineSpace font:(UIFont *)font textColor:(UIColor *)color
{
    ...
    self.linesSpacing = lineSpace/2.0;
    ...
    frame.size.height = [self getAttributedStringHeightWidthValue:width];
    self.frame = frame;
}

- (void)autoResetHeightByWidth:(CGFloat)restWidth lineSpace:(CGFloat)lineSpace
{
    if (!self.text || [self.text isEqualToString:@""]) {
        self.frame = CGRectZero;
        return;
    ...
    self.numberOfLines = 0;
    self.linesSpacing = lineSpace;
    CGRect frame = self.frame;
    frame.size.height = [self getAttributedStringHeightWidthValue:restWidth];
    self.frame = frame;
...
- (CGFloat)getAutoResetHeightByWidth:(CGFloat)width text:(NSString *)text font:(UIFont *)font lineSpace:(CGFloat)lineSpace
{
    ...
    self.text = text;
    return  [self getAttributedStringHeightWidthValue:width];
}
