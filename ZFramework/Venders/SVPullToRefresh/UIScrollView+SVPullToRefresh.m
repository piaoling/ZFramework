...
@property (nonatomic, assign) BOOL showPullAboveScrollView;
@property (nonatomic, assign) BOOL showsDateLabel;
@property (nonatomic, assign) BOOL isObserving;

@property(nonatomic, assign) CGFloat angle;   // default is CGAffineTransformIdentity. animatable
...
@dynamic pullToRefreshView, showsPullToRefresh,showPullAboveScrollView,timer;
...
- (void)setShowsPullAboveScrollView:(BOOL)showsPullAbove
{
    //showPullAboveScrollView = showsPullAbove;
}

- (BOOL)showPullAboveScrollView
{
    //return _showPullAboveScrollView;
    return YES;
}
...
@synthesize showPullAboveScrollView = _showPullAboveScrollView;
...
//                [self.activityIndicatorView stopAnimating];
                [self stopImageViewScrollAnimating];
...
//                [self.activityIndicatorView startAnimating];
                if (!self.timer) {
                    self.timer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(startImageViewScrollAnimating) userInfo:nil repeats:YES];
                }
                switch (self.position) {
                    case SVPullToRefreshPositionTop:
                ...
                break;
//            case SVPullToRefreshStateFinishLoading:
//                break;
        }
        ...
        //self.customImageView.image = [UIImage imageNamed:@"loading_squares.png"];
        ...
        CGFloat arrowX = (self.bounds.size.width / 2) - (totalMaxWidth / 2) + (leftViewWidth - self.customImageView.bounds.size.width) / 2 - 10;
        self.customImageView.frame = CGRectMake(arrowX,
                                      (self.bounds.size.height / 2) - (self.customImageView.bounds.size.height / 2),
                                      self.customImageView.bounds.size.width,
                                      self.customImageView.bounds.size.height);
    }
}
...
{
    if (!_customImageView) {
        _customImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 18, 18)];
        _customImageView.image = [UIImage imageNamed:@"loading_squares.png"];
        _customImageView.alpha = 1.0;
...
#pragma mark- CustomImageViewScroll Method
- (void)startImageViewScrollAnimating
{
    self.angle += M_PI/2 ;
    if (self.angle > 2* M_PI) {
        self.angle = 0;
    }
    _customImageView.transform = CGAffineTransformMakeRotation(self.angle);
}

- (void)stopImageViewScrollAnimating
{
...
- (void)stopAnimating
{
    [self performSelector:@selector(endPullAnimate) withObject:self afterDelay:2.0];
}
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    switch (newState) {
        ...
        case SVPullToRefreshStateLoading:
            ...
            break;
//        case SVPullToRefreshStateFinishLoading:
//            //self.titleLabel.text = self.titles[SVPullToRefreshStateFinishLoading];
//            break;
    }
}
